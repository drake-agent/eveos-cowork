...
function buildClientMetadata(redirectUrl: string): OAuthClientMetadata {
  return {
    client_name: 'EveOS Beauty MCP Connector',
...
export async function createOAuthCallbackListener(
  ...
  const server = createServer((request, response) => {
    ...
    if (authorizationCode) {
      ...
      response.end(
        '<html><body><h1>Authorization complete</h1><p>You can return to EveOS Beauty now.</p><script>setTimeout(() => window.close(), 1200);</script></body></html>'
      );
