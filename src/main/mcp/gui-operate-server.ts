...
// Get EveOS Beauty data directory for persistent storage
// Use platform-appropriate paths:
// - macOS: ~/Library/Application Support/EveOS Beauty
// - Windows: %APPDATA%/EveOS Beauty
const EVEOS_BEAUTY_DATA_DIR =
  PLATFORM === 'win32'
    ? path.join(
        process.env.APPDATA || path.join(os.homedir(), 'AppData', 'Roaming'),
        'EveOS Beauty'
      )
    : path.join(os.homedir(), 'Library', 'Application Support', 'EveOS Beauty');

// Directory for storing GUI operate files (screenshots, etc.)
const GUI_OPERATE_DIR = path.join(EVEOS_BEAUTY_DATA_DIR, 'gui_operate');
...
const GUI_APPS_DIR = path.join(EVEOS_BEAUTY_DATA_DIR, 'gui_apps');
  ...
  try {
    ...
    if (/Accessibility privileges not enabled/i.test(result.stderr || '')) {
      const hint =
        ...
        '- System Settings → Privacy & Security → Accessibility：允许 EveOS Beauty\n' +
        '- 如果是终端运行：允许 Terminal/iTerm\n' +
        '- 授权后请重启 EveOS Beauty 再重试\n';
      throw new Error(
    ...
    const hint =
      '\n\nmacOS 权限提示 / Permissions:\n' +
      '- System Settings → Privacy & Security → Accessibility：允许 EveOS Beauty\n' +
      '- System Settings → Privacy & Security → Automation：允许 EveOS Beauty 控制 “System Events”\n';
...
async function takeScreenshot(
  ...
  try {
    ...
    const hint =
      '\n\nmacOS 权限提示 / Permissions:\n' +
      '- System Settings → Privacy & Security → Screen Recording：允许 EveOS Beauty\n' +
