...
This document is the implementation spec for the fork. It intentionally does
not change Anna canonical data, but the client itself should be treated as an
EveOS Beauty product, not a lightly renamed upstream demo.
...
CoWork OS is useful as a product-pattern reference, but it is too broad as a
base migration for this goal. The parts to borrow are Mission Control-style
runtime visibility, timeline thinking, artifact/evidence workbench patterns,
workspace-kit/LLM-wiki ideas, and CLI/control-plane direction. The parts to
drop for this fork are broad "everything app" scope, email/docs/spreadsheets
...
5. Preserve useful upstream: keep OpenCowork's Electron, settings, session, and
...
7. Branded fork: visible app name, app icon, tray icon, favicon, shell copy,
   and Beauty workspace iconography should read as EveOS Beauty / BANILA CO,
   not OpenCowork.
...
### What to keep, replace, or drop

| Source | Keep | Replace | Drop for now |
|---|---|---|---|
| OpenCowork | Electron shell, IPC/preload, encrypted config, SQLite/local persistence, tests, packaging, generic chat fallback. | Product name, app icon, visible shell labels, Beauty entry surface, Beauty API token flow, answer contract. | Any direct Anna/OpenClaw access from the client. |
| CoWork OS | Mission Control concept, queue/timeline visibility, artifact/evidence workbench idea, CLI/control-plane direction for later. | Broad runtime concepts into Beauty-specific queue, evidence packet, and analyst run UX. | Everything-app scope, mail/docs/sheets/decks as first-class MVP surfaces, separate local daemon that competes with Anna. |
| BANILA/F&CO | Rose/cream/charcoal palette, "Zero to Glow" metaphor, refined urban feminine restraint, product-planning cockpit density. | Generic AI SaaS visuals with an evidence-first Beauty OS language. | Cute/kitsch/girly/neon/slang, decorative blobs, unrelated AI-purple gradients. |
...
- TikTok crawling.
- Apify.
- A broad feature rewrite of every upstream screen before the core Beauty flow works.
...
### Phase 3: Taste pass, product identity, and packet history
...
- Visible app shell, app metadata, icons, and tray identity no longer present
  as upstream OpenCowork.
...
- Apply BANILA/F&CO theme tokens and EveOS Beauty product identity to visible
  app shell surfaces, icons, and packaging metadata.

Do not attempt a whole-app feature rewrite in the first PR.
